...
namespace Strafe
{
	...
	{
		double oldspeedsqr = DotProduct<float, float, 2>(strafedata.player.Velocity, strafedata.player.Velocity);
		double theta = ConstSpeedTheta(strafedata, wishspeed);
	...
	void StrafeVectorial(StrafeData& strafedata, ProcessedFrame& out, bool yawChanged)
	{
		...
		if (dummy.Forward)
		{
			if (!yawChanged && strafedata.frame.VectorialIncrement > 0)
			{
				...
				double additionAbs = min(static_cast<double>(strafedata.frame.VectorialIncrement), std::abs(normalizedDiff));
...
#pragma warning(pop)
	...
	void Friction(StrafeData& strafedata)
	{
		...
		auto newspeed = max(speed - drop, 0.f);
